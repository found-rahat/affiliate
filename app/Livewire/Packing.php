...
class Packing extends Component
{
    ...
    public function submitOrderNumber(){
        ...
        if ($order->status === 'Processing') {
            if ($order->packing_type === null) {
                $order->packing_type = 'Pending';
                $order->packing_time = Carbon::now();
                $order->packing_user = Auth::user()->name;
                $order->save();

                // Clear and rebuild pending orders list
                $this->pendingOrders = CustomerInfo::with('items.product')->where('packing_type', 'Pending')->latest()->get()->toArray();
    ...
    public function PackingList(){
        ...
        Notification::make()->success()->title('Packing Confirm')->body('Confirmed Packing orders successfully.')->send();
        $this->dispatch('reloadPage');
    }
