...
<div class="container">
    <div class="page-inner">
        ...
        @if ($cartItems->isNotEmpty())
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        @foreach ($cartItems as $item)
                            ...
                            @endphp
                            <tr>
                                <td>{{ $item->product->id }}{{ Str::limit($item->product->product_name, 30) }}</td>
                                <td>
                                ...
                                <td>
                                    <input type="number" name="prices[{{ $item->id }}]"
                                        value="{{ $item->sell_price }}"step="0.01" min="0"
                                        class="form-control" />
                                </td>
                                <td>
                                    <input type="number"
                                        name="quantities[{{ $item->id }}]"value="{{ $item->quentity }}"
                                        min="1" class="form-control" />
                                </td>
                                <td><i class="fa-solid fa-bangladeshi-taka-sign"></i> {{ number_format($total, 2) }}
                                </td>
                        ...
                        <td></td> --}}
                            ...
                            <td><i class="fa-solid fa-bangladeshi-taka-sign"></i> {{ number_format($grandTotal, 2) }}
                            </td>
            ...
            <form action="{{ route('user.order.submit') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card p-4">
        ...
        @else
            <h4> No Add to cart. continue shopping </h4>
        @endif
